...
def link_address_transaction(address, chain_id):
    ...
    url = "https://api.covalenthq.com/v1/{}/address/{}/portfolio_v2/?key={}".format(
    ...
    for i in range(len(r)):
        ...
        if (
            r != "0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee"
            and r != "0xzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz"
            and r != "0xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
        ):
            list_contract_address.append(r)

    df2 = pd.DataFrame()
    for contract_address in list_contract_address:
        url1 = "https://api.covalenthq.com/v1/{}/address/{}/transfers_v2/?contract-address={}&key={}".format(
            chain_id, address, contract_address, api_key
        )
    ...
    nbr1 = 1
    while r1 == True:
        url1 = "https://api.covalenthq.com/v1/{}/address/{}/transfers_v2/?page-number={}&contract-address={}&key={}".format(
            chain_id, address, nbr1, contract_address, api_key
        )
        r2 = rq.get(url1).json()["data"]["items"]
        if r2 != []:
            df2 = df2.append(r2, ignore_index=True)
