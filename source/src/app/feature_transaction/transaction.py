...
import pandas as pd
from src.app.feature_transaction.link_address_transaction import (
    link_address_transaction,
)
...
def transaction(address, chain_id):
    ...
    for i in range(len(df)):
        ...
        if x["transfers"][0]["transfer_type"] == "OUT":
            ...
            y = int(x["transfers"][0]["delta"]) * (
                10 ** (-x["transfers"][0]["contract_decimals"])
            )
            ...
            for i in transaction["Balance"]:
                if i != ".":
                    ...
                    if 5 <= compte <= 8:  # -1 000 = -1K
                        transaction["Balance"] = (
                            transaction["Balance"][: compte - 3]
                            + "."
                            + transaction["Balance"][compte - 3 : compte]
                            + " k"
                        )
                    elif 8 <= compte <= 11:  # -1 000 000 = -1M
                        transaction["Balance"] = (
                            transaction["Balance"][: compte - 6]
                            + "."
                            + transaction["Balance"][compte - 6 : compte - 3]
                            + " M"
                        )
                    elif 12 <= compte:
                        transaction["Balance"] = format(
                            float(transaction["Balance"]), ".1E"
                        )

            if x["transfers"][0]["delta_quote"] == None:
                transaction["Holdings (en USD)"] = "None"
            else:
                transaction["Holdings (en USD)"] = format(
                    -x["transfers"][0]["delta_quote"], ".5f"
                )
        else:
